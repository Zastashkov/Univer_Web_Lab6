function power( x,  p){
   ...
   if (ans != 1)document.getElementById('output').textContent = ans;
}
function NOD(x,y){
    let max = 0;
    var ans = 0;
    ...
    if(max != 0 )document.getElementById('NOD').textContent = ans; 
}
function MinV(value){
    ...
    for(let i = 0; i < array.length; i++)
     ...
     if(min != 100)document.getElementById('answer').textContent = min;
}
...
function fibon(count){
    ...
    if(res != 0)document.getElementById('answerers').textContent = res;
}